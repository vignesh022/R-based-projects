...
fit2=glm(respdeath~factor(agegr)+period+factor(arsenic)+start,offset=log(personyrs),family="poisson",data=Montana)
summary(fit2)
...
fit2=glm(respdeath~factor(agegr)+factor(period)+factor(arsenic)+start,offset=log(personyrs),family="poisson",data=Montana)
summary(fit2)
...
pred.mean=predict(fit2,newdata=data,type="response",se=T)
pred.mean
